...
Worker execution flags also default to false: `PET_ENABLE_BROWSER_NAVIGATION`, `PET_ENABLE_MUSIC_PLAYBACK`, and `PET_ENABLE_DEVICE_MODEL_CHAT`. The worker consumes only `pet_capability_signed_execution`, verifies its target/signature/executor mapping, checks for an existing receipt before invoking a handler, records the signed receipt, and acknowledges the exact speaker message. Host packages must register handlers explicitly; no handler ships enabled by default.
