...
- `PET_BROWSER_ALLOWED_DOMAINS=` is empty by default and fails closed. Configure a reviewed comma-separated public-domain allowlist before browser requests can be accepted.
...
Set per-machine directional keys instead of one shared secret: Brain dispatch uses `PET_DISPATCH_SIGNING_KEY_<MACHINE_ID>`, target workers verify with `PET_DISPATCH_VERIFY_KEY_<MACHINE_ID>`, workers sign receipts with `PET_RECEIPT_SIGNING_KEY_<MACHINE_ID>`, and Brain verifies receipts with `PET_RECEIPT_VERIFY_KEY_<MACHINE_ID>`. Migration 018 adds managed key IDs, fingerprint-only lifecycle authority, expiry/revocation events, restart-durable nonce consumption, and a transactional/idempotent speaker outbox. These are intentionally unset by default, so dispatch and receipt ingestion fail closed. See `PET_MACHINE_RUNTIME_SECURITY_RUNBOOK.md` for generation and rotation.
